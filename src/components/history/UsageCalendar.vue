    ...
    <div class="py-2 text-sm">
        <div class="p-3">
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="min-w-10">
                    <button v-show="showPrevious" class="bg-white px-2 py-1 rounded-md cursor-pointer shadow"
                        @click="previous">
                ...
                <div class="min-w-10">
                    <button v-show="showNextBtn" class="bg-white px-2 py-1 rounded-md cursor-pointer shadow"
                        @click="next">
            ...
            <div ref="swipe-catcher" class="grid grid-cols-7 gap-1 mt-1">
                <div v-for="day in calendarDays" :key="day.key" class="min-h-14 p-0.5 rounded overflow-hidden" :class="{
                    ...
                    'bg-amber-500 text-white': day.histories.length > 0 && day.currentMonth,
                    'bg-gray-200 text-white': day.histories.length > 0 && !day.currentMonth,
                    'bg-gray-50 ': day.histories.length == 0,
                }">
                    <div class="text-xs text-center font-medium p-0.5" :class="{
                        'text-gray-400': !day.currentMonth && day.histories.length == 0,
                    }">
                        {{ day.day }}
                    </div>
                    ...
                    <div class="mt-1 space-y-1">
                        <div v-for="history in day.histories.slice(0, 3)" :key="history.ts"
                            class="rounded px-0 py-0.5 font-semibold text-[8px] truncate text-white text-center"
                            :title="new Date(history.ts).toLocaleString()">
                            {{ formatTime(history.ts) }}
                        </div>

                        <div v-if="day.histories.length > 3" class="text-[8px] text-gray-500 px-1">
...
<script setup lang="ts">
    ...
    const showPrevious = computed(() => {
        return month.value > earlistMonth.value.month && year.value >= earlistMonth.value.year
    })

    const earlistMonth = computed(() => {
